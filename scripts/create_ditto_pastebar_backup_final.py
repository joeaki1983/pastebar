#!/usr/bin/env python3
"""
最终版本：完全基于成功案例的 Ditto 到 PasteBar 数据转换脚本
"""
...
def generate_pastebar_uuid():
    """生成与成功案例完全匹配的UUID格式"""
    # 成功案例使用类似 "EXog1zZzYpkV8R7ZCvWCZ" 的格式
...
def process_ditto_data(ditto_db_path, pastebar_conn):
    """处理 Ditto 数据库并转换为 PasteBar 格式 - 完全匹配成功案例"""
    ...
    for record in ditto_records:
        ...
        # 使用快速粘贴文本作为标题，如果没有则留空（与成功案例一致）
        if quick_paste_text and quick_paste_text.strip():
            title = quick_paste_text
        else:
            title = ""  # 成功案例中很多记录的title为空
...
def main():
    parser = argparse.ArgumentParser(description='创建 PasteBar 兼容的备份文件 (最终版本)')
